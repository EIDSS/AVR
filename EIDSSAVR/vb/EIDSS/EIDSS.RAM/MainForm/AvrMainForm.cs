...
namespace eidss.avr.MainForm
{
    public partial class AvrMainForm : BaseAvrForm, IAvrMainFormView
    {
        ...
        private void AvrMainForm_Paint(object sender, PaintEventArgs e)
        {
            foreach (BarItemLink link in barLanguageSubItem.ItemLinks)
            {
                BarItem item = link.Item;
                //  link.Visible = Localizer.AllSupportedLanguages.ContainsKey(link.Item.Tag.ToString());
                if (Localizer.AllSupportedLanguages.ContainsKey(item.Tag.ToString()) == false)
                {
        ...
        //    bool bWithouFlags = true;  //     IsLangMenuWithoutFlags;
        ...
        //        BarButtonItem biLanguage = new BarButtonItem(barManager,
        //            Localizer.GetMenuLanguageName(lang.Key)                 ;
        //        barMenu.ItemLinks.Add(biLanguage);             

        //        biLanguage.ItemClick+=  x(
            ...
            try
            {
                using (CreateWaitDialog())
                {
                    ...
                    if (found == null)
                    {
                        ...
                        Control c = (Control)avrForm;
                        // avrForm.WindowState = FormWindowState.Minimized;
                        if (avrForm.ParentForm != null)
                        {
                            avrForm.ParentForm.MinimumSize = avrForm.MinimumSize;
                        }
        ...
        private void BarButtonItem2_ItemClick_1(object sender, ItemClickEventArgs e)
        {
            ...
            EIDSS.PreferenciesForm.ShowMe();

        }


        private void biLanguage_ItemClick(object sender, ItemClickEventArgs e)
        {
            using (new WaitDialog(WaitDialogType.FormLoading))
            {
                MenuActionManager.Instance.MainFormInstance.ResetLanguage(e.Item.Tag.ToString());
